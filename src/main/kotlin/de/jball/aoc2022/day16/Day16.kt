...
class Day16(test: Boolean = false) : AdventOfCodeDay<Int>(test, 1651, 1707) {
	...
	private fun currentElephantValveIsNotOpen(lastState: StateWithElephant) =
		!lastState.openValves.contains(lastState.elephantPosition)

	private fun currentValveHasImpact(lastState: SingleState) = valvesWithImpact.containsKey(lastState.myPosition)
	private fun currentElephantValveHasImpact(lastState: StateWithElephant) =
		valvesWithImpact.containsKey(lastState.elephantPosition)

	private fun openVent(current: SingleState) = SingleState(
		current.openValves.plus(current.myPosition),
		current.ventedSteam + current.openValves.sumOf { valvesWithImpact[it]!! },
		current.myPosition
	)

	private fun openVent(current: StateWithElephant) = StateWithElephant(
		current.openValves.plus(current.myPosition),
		current.ventedSteam + current.openValves.sumOf { valvesWithImpact[it]!! },
		current.myPosition, current.elephantPosition
	)

	private fun openElephantVent(current: StateWithElephant) = StateWithElephant(
		current.openValves.plus(current.elephantPosition),
		current.ventedSteam,
		current.myPosition, current.elephantPosition
	)
	...
	private fun move(current: StateWithElephant): Set<StateWithElephant> {
		...
		newStates.forEach { newState ->
			happenedStatesWithElephant[Triple(newState.openValves, newState.myPosition, newState.elephantPosition)] =
				newState.ventedSteam
		}
	...
	private fun didntHappenOrWasWorse(state: StateWithElephant) =
		!happenedStatesWithElephant.containsKey(Triple(state.openValves, state.myPosition, state.elephantPosition)) ||
			happenedStatesWithElephant[Triple(
				state.openValves,
				state.myPosition,
