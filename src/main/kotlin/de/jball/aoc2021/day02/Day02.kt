...
class Day02(test: Boolean = false) : AdventOfCodeDay<Long>(test, 150, 900) {
	...
	override fun part2(): Long {
		val aims = preparedInput.runningFold(0L) { aim,
												   pair ->
			when (pair.first) {
		...
		val depthHor = preparedInput.foldIndexed(Pair(0L, 0L)) { index, depthHor, currentInput ->
			if ("forward" == currentInput.first)
				Pair(
					depthHor.first + aims[index] * currentInput.second,
