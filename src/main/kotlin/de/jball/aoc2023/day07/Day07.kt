...
class Day07(test: Boolean = false) : AdventOfCodeDay<Int>(test, 6440, 5905) {
	...
	private fun partOneCompareCardAt(one: Hand, other: Hand, index: Int) =
		one.cards[index].compareTo(other.cards[index])

	private fun partTwoCompareCardAt(one: Hand, other: Hand, index: Int) =
		partTwoCardOrder.indexOf(one.cards[index].label)
