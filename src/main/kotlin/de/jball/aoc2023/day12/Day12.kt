...
class Day12(test: Boolean = false) : AdventOfCodeDay<Long>(test, 21, 525152) {
	...
	fun possibleArrangements(damageReport: String, groupSizes: List<Int>): Long {
		...
		if (damageReport.startsWith("#")) {
			return if (Regex("^[#?]{${groupSizes.first()}}[^#]").containsMatchIn(damageReport)) {
				if (damageReport.length < groupSizes.first() + 1) {
					...
					cache[Pair(damageReport.substring(groupSizes.first() + 1), groupSizes.drop(1))] =
		...
		if (damageReport.startsWith("?")) {
			...
			val combinationsIfFirstSpringBroken =
				possibleArrangements(damageReport.replaceFirstChar { '#' }, groupSizes)
	...
	private fun unfold(annotatedReport: Pair<String, List<Int>>) =
		Pair(unfold(annotatedReport.first), unfold(annotatedReport.second))
}
